...
function getArtistByIndex(artistsArr,searchIndex) {
  const artist = artistsArr[searchIndex]
...
function listOfNames(artistsArr) {
  const artistNamesArr = artistsArr.map( item => item.name)
...
function removeArtist(artistsArr,removeIndex) {
  return artistsArr.filter( item => item !== artistsArr[removeIndex])
}
...
function addArtist(artistsArr,name,years,genre,nationality,bio) {
  const copiedArtistArr = [...artistsArr]
  copiedArtistArr.push({name,years,genre,nationality,bio})
  return copiedArtistArr
}
...
function lotsOfArt(artistsArr) {
  return artistsArr.filter(item => item.paintings > 100).map(item => item.name)
}
...
function artistInfo(artistsArr,artistName){
  return artistsArr.filter( item => item.name === artistName).map( item => item.bio)[0]
}
...
function artistByCountry(artistsArr,searchNationality){
  return artistsArr.filter( item => item.nationality.split(",").length < 2)
  .filter( item => item.nationality === searchNationality)
  .map( item => item.name)
}
